...
/**
 * Calculates the world-space coordinates of a specific anchor for a shape,
...
/**
 * Returns all 5 anchor points in world-space coordinates for a given shape.
 */
...
/**
 * Checks if a shape is a valid target for connector attachments in v1.
 * Connectors can attach only to primitive shapes: rectangle, text, sticky_note.
 */
...
export function findNearestAnchor(
  ...
  for (const shape of shapes) {
    ...
    // For rotated shapes, expand conservatively by max(width, height)
    ...
    for (const anchor of ANCHOR_POSITIONS) {
      const anchorPoint = getShapeAnchorPoint(shape, anchor);
