import { AlertTriangle, Clock, History, RefreshCw, RotateCcw, User, X } from "lucide-react";
...
export function VersionPreviewModal({
  ...
  const handleConfirmRestore = async (): Promise<void> => {
    ...
    try {
      await restore({ versionId: versionDetail.id });
    } catch {
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent): void => {
      if (e.key === "Escape" && !isConfirmingRestore) {
        e.stopPropagation();
        closePreview();
      }
    };
  ...
  }, [previewVersionId, isConfirmingRestore, closePreview]);
  ...
  return (
    <>
      <div
        ...
        onClick={() => {
          if (!isConfirmingRestore) closePreview();
        }}
      >
        <div
        ...
        >
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 px-5 py-3.5 border-b border-slate-800 bg-slate-900/90 backdrop-blur-md shrink-0">
            ...
            {/* Right: Restore & Close Action */}
            <div className="flex items-center gap-2">
              {versionDetail && !isLoading && !isError && (
                <button
                  type="button"
                  onClick={() => setIsConfirmingRestore(true)}
                  title={`Restore Version ${versionDetail.versionNumber}`}
                  aria-label={`Restore Version ${versionDetail.versionNumber}`}
                  data-testid="preview-restore-btn"
                  className="inline-flex items-center gap-1.5 rounded-lg bg-blue-600 px-3 py-1.5 text-xs font-semibold text-white shadow-xs hover:bg-blue-500 transition-colors focus:outline-none focus:ring-2 focus:ring-blue-400 cursor-pointer"
                >
              ...
              <button
                ...
                className="rounded-lg p-2 text-slate-400 hover:bg-slate-800 hover:text-slate-100 transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500 cursor-pointer"
              >
                <X className="h-5 w-5" />
              </button>
            </div>
      ...
      <RestoreConfirmationModal
        ...
        versionName={versionDetail?.name}
